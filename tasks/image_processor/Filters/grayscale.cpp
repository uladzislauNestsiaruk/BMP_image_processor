...
#include "grayscale.h"
void GrayscaleFilter::apply(BMPStream& bmp_stream) {
    for(int32_t y_coordinate = 0; y_coordinate < bmp_stream.GetDibHeader().height; ++y_coordinate){
        for(int32_t x_coordinate = 0; x_coordinate < bmp_stream.GetDibHeader().width; ++x_coordinate){
            ...
            uint8_t pixel_value = static_cast<uint8_t>(RDELTA * static_cast<double>(pixel.R) + BDELTA * static_cast<double>(pixel.B) +
